...
cdef extern from "indexing.hpp":
    ...
    PointXY  *getptr(PointCloud[PointXY] *, size_t)
    PointXYZ *getptr_at(PointCloud[PointXYZ] *, size_t) except +
    PointXY  *getptr_at(PointCloud[PointXY] *, size_t) except +
    PointXYZ *getptr_at(PointCloud[PointXYZ] *, int, int) except +
    PointXY  *getptr_at(PointCloud[PointXY] *, int, int) except +
...
##    enum UpsamplingMethod "pcl::MovingLeastSquares<pcl::PointXYZ, pcl::PointXYZ>::UpsamplingMethod":
##            NONE
##            DISTINCT_CLOUD
##            SAMPLE_LOCAL_PLANE
...
ctypedef KdTreeFLANN[PointXY] KdTreeFLANN2_t

cdef extern from "pcl/registration/correspondence_rejection_trimmed.h" namespace "pcl::registration" nogil:
  cdef cppclass CorrespondenceRejectorTrimmed:
    CorrespondenceRejectorTrimmed() except +
    void setOverlapRatio(float ratio) except +
    float getOverlapRatio() except +

ctypedef shared_ptr[CorrespondenceRejectorTrimmed] CorrespondenceRejectorTrimmedPtr_t
ctypedef vector[CorrespondenceRejectorTrimmedPtr_t] CorrespondenceRejectorTrimmedVector_t
